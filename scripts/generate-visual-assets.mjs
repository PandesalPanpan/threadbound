...
// These are the source exports from the approved Figma battle concept. They are
// converted to the same content-addressed runtime format as the generated sheets
// so every consumer still resolves art through the semantic catalog.
const FIGMA_BATTLE_ASSETS = Object.freeze([
  { source: 'threadbound-figma-bramble-druid.v1.svg', nodeId: '112:7', kind: 'character', id: 'character.bramble-druid-figma.v1', label: 'Bramble Druid', tags: ['character', 'battle', 'player', 'support'] },
  { source: 'threadbound-figma-iron-vanguard.v1.svg', nodeId: '112:12', kind: 'character', id: 'character.iron-vanguard-figma.v1', label: 'Iron Vanguard', tags: ['character', 'battle', 'player', 'tank'] },
  { source: 'threadbound-figma-rune-bard.v1.svg', nodeId: '112:17', kind: 'character', id: 'character.rune-bard-figma.v1', label: 'Rune Bard', tags: ['character', 'battle', 'player', 'arcane'] },
  { source: 'threadbound-figma-cinder-imp.v1.svg', nodeId: '112:22', kind: 'mob', id: 'mob.cinder-imp-figma.v1', label: 'Cinder Imp', tags: ['mob', 'battle', 'enemy', 'fire'] },
  { source: 'threadbound-figma-rot-toad.v1.svg', nodeId: '112:27', kind: 'mob', id: 'mob.rot-toad-figma.v1', label: 'Rot Toad', tags: ['mob', 'battle', 'enemy', 'poison'] },
  { source: 'threadbound-figma-gloom-hound.v1.svg', nodeId: '112:32', kind: 'mob', id: 'mob.gloom-hound-figma.v1', label: 'Gloom Hound', tags: ['mob', 'battle', 'enemy', 'shadow'] },
]);
...
for (const authored of FIGMA_BATTLE_ASSETS) {
  const source = path.join(OUTPUT_DIR, authored.source);
  const rendered = await sharp(source)
    .resize({ width: 256, height: 256, fit: 'inside', withoutEnlargement: true, kernel: sharp.kernel.nearest })
    .webp({ lossless: true, effort: 6 })
    .toBuffer({ resolveWithObject: true });
  const hash = createHash('sha256').update(rendered.data).digest('hex').slice(0, 12);
  const name = `battle-${slug(authored.label)}.${hash}.webp`;
  await writeFile(path.join(OUTPUT_DIR, name), rendered.data);
  assets.push({
    id: authored.id,
    kind: authored.kind,
    label: authored.label,
    description: `Figma battle concept artwork for ${authored.label}.`,
    tags: authored.tags,
    src: `/assets/runtime/${name}`,
...
const renderedCharacterLibrarySources = new Map();
for (const authored of FIGMA_CHARACTER_LIBRARY) {
  const source = path.join(SOURCE_DIR, 'figma-character-library', 'v1', authored.sourceFilename);
  let rendered = renderedCharacterLibrarySources.get(authored.sourceFilename);
  if (!rendered) {
    rendered = await sharp(source)
      ...
      .toBuffer({ resolveWithObject: true });
    const hash = createHash('sha256').update(rendered.data).digest('hex').slice(0, 12);
    const name = `character-library-${slug(authored.sourceName)}.${hash}.webp`;
    await writeFile(path.join(OUTPUT_DIR, name), rendered.data);
    rendered = { ...rendered, name };
    renderedCharacterLibrarySources.set(authored.sourceFilename, rendered);
  }

  for (const kind of authored.allowedKinds) {
    const visualAssetId = `${kind}.${slug(authored.sourceName)}.v1`;
    assets.push({
      id: visualAssetId,
      kind,
      label: authored.label,
      description: `Figma ${authored.boardCategory} artwork for ${authored.label} (${authored.role}).`,
      family: authored.family,
      role: authored.role,
      tags: [...new Set([kind, ...authored.tags])],
      src: `/assets/runtime/${rendered.name}`,
      width: rendered.info.width,
      height: rendered.info.height,
      provenance: {
      ...
      sourceMaster: {
        ...
        boardCategory: authored.boardCategory,
        file: `public/assets/generated/figma-character-library/v1/${authored.sourceFilename}`,
