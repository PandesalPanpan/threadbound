...
export function BattleArena({ payload, assets = [], frame, phase, active = null, busy, onStart, onReplay, onBack }) {
  ...
  return (
    <section className="battle-card automatic-battle-card" data-testid="battle-card" data-battle-phase={phase} data-authoritative="true">
      <header className="battle-card__header"><div><span className="battle-card__eyebrow">FIGMA BATTLE FLOW · SERVER REPLAY</span><h1>Threadbound battle</h1></div><div className="battle-card__status" data-testid="battle-phase-label"><span className="status-dot" />{phaseLabel}</div></header>
      {phase === 'preBattle' ? (
        <div className="battle-empty battle-prebattle" data-testid="battle-pre-battle">
          <span className="battle-card__eyebrow">01 · PRE-BATTLE</span><div className="battle-empty__sigil" aria-hidden="true"><span>✦</span></div><h2>Watch the threads fight</h2><p>Three Weavers face three threats. Start the server-resolved simulation, then follow every committed action in one focused arena.</p>
          <div className="battle-roster-preview" aria-label="Battle roster"><div><span className="battle-roster-label">WEAVERS</span>{roster.players.map((unit) => <div className="battle-roster-unit" key={unit.id}><img src={unit.asset?.src} alt="" data-visual-asset-id={unit.asset?.id} /><span>{unit.label}</span></div>)}</div><span className="battle-roster-vs">VS</span><div><span className="battle-roster-label">THREATS</span>{roster.enemies.map((unit) => <div className="battle-roster-unit" key={unit.id}><img src={unit.asset?.src} alt="" data-visual-asset-id={unit.asset?.id} /><span>{unit.label}</span></div>)}</div></div>
          <ActionButton onClick={onStart} disabled={busy} testId="battle-start" variant="primary">Start simulation</ActionButton><button className="quiet-link" type="button" onClick={onBack}>Return to the Adventure Stream</button>
        </div>
      ) : (
        <>
          <div className={`battle-stage battle-stage--${phase}`} data-testid="battle-stage" aria-label={`${phaseLabel} battle arena`}>
            ...
            <section className="battle-team battle-team--enemies" data-testid="battle-enemies" data-battle-team="enemies" aria-label="Enemy team"><span className="battle-team__label">ENEMY THREAD · {teams.enemies.some((unit) => Number(unit.hp) <= 0) ? 'UNDER PRESSURE' : '3 HOSTILES'}</span><div className="battle-team__units">{teams.enemies.map((unit) => <BattleUnit key={unit.id} unit={unit} side="target" active={activeActor === unit.id} targeted={activeTarget === unit.id} phase={phase} />)}</div></section>
            <div className="battle-threadline" aria-hidden="true"><span /><i /><b /></div>
            <section className="battle-team battle-team--players" data-testid="battle-players" data-battle-team="players" aria-label="Player team"><span className="battle-team__label">YOUR THREAD · {teams.players.some((unit) => Number(unit.hp) <= 0) ? 'HOLDING' : '3 WEAVERS'}</span><div className="battle-team__units">{teams.players.map((unit) => <BattleUnit key={unit.id} unit={unit} side="attacker" active={activeActor === unit.id} targeted={activeTarget === unit.id} phase={phase} />)}</div></section>
          ...
          <div className="battle-readout"><div><span>TURN</span><strong>{frame?.currentTurn || payload?.battle?.turns?.length || 0}/{payload?.battle?.turns?.length || 0}</strong></div><div><span>ROSTER</span><strong>3 × 3</strong></div><div><span>STATUS</span><strong>{phaseLabel}</strong></div></div>
