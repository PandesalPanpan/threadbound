...
test('owner and spectator share the same Blackjack table and public Dungeon replay', async ({ browser }) => {
  ...
  try {
    ...
    await expect(ownerDungeon.getByTestId('shared-battle-surface')).toHaveAttribute('data-replay-state', 'complete');
    await expect(observerDungeon.getByTestId('shared-battle-surface')).toHaveAttribute('data-replay-state', 'complete');
