...
test('external Arc Manifest can be uploaded, validated, published, played, and documented', async ({ page }) => {
  ...
  await expect(page.getByTestId('workshop-status')).toContainText('Saved The Ashen Thread revision 1 as a draft');
  ...
  await expect(page.getByTestId('workshop-status')).toContainText('Published The Ashen Thread revision 1');
  ...
  await expect(cinderDungeon).toContainText('manifest r1');
  ...
  await expect(page.getByTestId('app-status')).toHaveText('Ready');
  await expect(page.getByTestId('inventory-item')).toContainText('Ember Needle of the Loom');
  await expect(page.getByTestId('inventory-item')).toContainText('+3 attack');
  await expect(page.getByTestId('achievement')).toContainText('Through the Cinders');
